...
abstract class VcLiteralImplMixin(node: ASTNode) : VcExprImplMixin(node), VcLiteral {
    override fun <P : Any?, R : Any?> accept(visitor: AbstractExpressionVisitor<in P, out R>, params: P?): R {
        longName?.let { return visitor.visitReference(it, LongUnresolvedReference(it, it.prefixName.textRepresentation(), it.refIdentifierList.map { it.referenceName }), params) }
