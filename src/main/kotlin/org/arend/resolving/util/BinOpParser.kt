...
fun resolveReference(data: Any?, referent: Referable, fixity: Fixity?, scope: Scope? = null) =
        if (data is ArendCompositeElement) {
            val refExpr = Concrete.FixityReferenceExpression.make(data, referent, fixity, null, null)
            val arg = ExpressionResolveNameVisitor.resolve(refExpr, scope ?: ((data as? ArendIPName)?.parentLiteral ?: data).scope, false, null)
...
private fun getExpression(expr: Abstract.Expression?, scope: Scope? = null): Concrete.Expression {
    val ref = expr?.accept(object : BaseAbstractExpressionVisitor<Void, Concrete.Expression?>(null) {
        ...
        override fun visitReference(data: Any?, referent: Referable, fixity: Fixity?, pLevels: Collection<Abstract.LevelExpression>?, hLevels: Collection<Abstract.LevelExpression>?, params: Void?) = resolveReference(data, referent, fixity, scope)
    }, null)
