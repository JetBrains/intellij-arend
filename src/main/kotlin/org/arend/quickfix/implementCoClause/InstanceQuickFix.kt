...
private fun findImplementedCoClauses(coClauseList: List<CoClauseBase>,
    ...
    for (coClause in coClauseList) {
        ...
        if (!fields.remove(referable)) holder?.apply {
            newAnnotation(HighlightSeverity.ERROR, "Field ${referable.textRepresentation()} is already implemented")
                    .range(BasePass.getImprovedTextRange(null, coClause))
                    .newFix(RemoveCoClauseQuickFix(SmartPointerManager.createPointer(coClause)))
                    .registerFix()
                    .create()
        }
    ...
    for (coClause in coClauseList) {
        ...
        if (referable is ClassReferable) {
            ...
            if (subClauses.isEmpty() && fatArrow == null) {
                holder.newAnnotation(HighlightSeverity.WEAK_WARNING, "Coclause is redundant")
                ...
                when (annotationToShow) {
                    InstanceQuickFixAnnotation.IMPLEMENT_FIELDS_ERROR -> {
                        ...
                        holder.newAnnotation(HighlightSeverity.ERROR, message)
                                .range(rangeToReport)
                                .newFix(ImplementFieldsQuickFix(SmartPointerManager.createPointer(classReferenceHolder), annotationToShow != InstanceQuickFixAnnotation.IMPLEMENT_FIELDS_ERROR, fieldsList))
                                .registerFix()
                                .create()
                    }
