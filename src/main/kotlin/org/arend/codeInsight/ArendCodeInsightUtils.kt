    ...
    companion object {
            ...
            if (constructor.parent?.parent is ArendDefData) { //Case I: Simple constructor
                ...
                if (dataNewParameters != null) {
                    ...
                    for ((index, nP) in dataNewParameters.withIndex()) {
                        ...
                        if (nP.isExternal() && firstInternalIndex != -1) throw AssertionError()

                        val oldIndex = if (nP.oldParameter == null) -1 else {
                            val i = dataOldParameters.indexOf(nP.oldParameter)
                        ...
                        newParameters.add(ParameterDescriptor.createDataParameter_(oldParameter, nP.getExternalScope(), nP.getNameOrNull(), nP.getType1()))
                    }
                }
                        ...
                        while (i < j) {
                            ...
                            if (oldArgument != null) {
                                ...
                                val parameter = if (referable is Referable)
                                    ...
                                    ParameterDescriptor.createUnnamedDataParameter(oldArgument.getType1())
                ...
                if (dataNewParameters != null) {
                    ...
                    for ((index, nP) in dataNewParameters.withIndex()) {
                        ...
                        if (oldParameter == null) { // newly created internal data parameter, there will be an autogenerated "_" dummy pattern for it
                            newParameters.add(ParameterDescriptor.createDataParameter_(null, nP.getExternalScope(), nP.getNameOrNull(), nP.getType1()))
                        } else {
                            ...
                            newParameters.addAll(patternParameterDescriptors.map {
                                ParameterDescriptor.createDataParameter_(it, nP.getExternalScope(), nP.getNameOrNull(), nP.getType1())})
                        }
                    }
                }
            }
        ...
        fun getParameterList(def: Abstract.ParametersHolder, addTailParameters: Boolean = false): Pair<List<ParameterDescriptor>?, Boolean> {
            ...
            val parameters = when (def) {
                ...
                is ArendDefClass -> externalParametersOrEmpty + getClassParameterList(def)
                        ...
                        when (tele) {
                            is ArendTypeTele ->
                                if (tele.typedExpr?.identifierOrUnknownList.isNullOrEmpty()) singletonList(null) else
                                    tele.typedExpr?.identifierOrUnknownList!!.map { iou -> iou.defIdentifier?.let { defIdentifier -> ParameterDescriptor.createExternalParameter(defIdentifier, tele.isExplicit, tele.type) } }

                            is ArendNameTele ->
                                if (tele.identifierOrUnknownList.isEmpty()) singletonList(null) else
                                    tele.identifierOrUnknownList.map { iou -> iou.defIdentifier?.let { defIdentifier -> ParameterDescriptor.createExternalParameter(defIdentifier, tele.isExplicit, tele.type) } }
        ...
        private fun getTailParameters(resType: ArendExpr?): List<ParameterDescriptor> {
            ...
            while (resTypeVar != null) {
                resTypeVar = when (resTypeVar) {
                    is ArendArrExpr -> {
                        result.add(ParameterDescriptor(true, resTypeVar.domain?.text, null))
                        resTypeVar.codomain
                    }
                    is ArendPiExpr -> {
                        result.addAll(resTypeVar.parameters.map { tele -> tele.referableList.map { if (it != null) ParameterDescriptor(it) else ParameterDescriptor.createUnnamedParameter(tele) } }.flatten())
                        resTypeVar.codomain
                    }
