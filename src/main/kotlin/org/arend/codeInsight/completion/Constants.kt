...
val STATEMENT_WT_KWS = listOf(FUNC_KW, SFUNC_KW, LEMMA_KW, TYPE_KW, CONS_KW, DATA_KW, CLASS_KW, RECORD_KW, INSTANCE_KW, OPEN_KW, MODULE_KW, META_KW).map { it.toString() }
val CLASS_MEMBER_KWS = listOf(FIELD_KW, PROPERTY_KW, OVERRIDE_KW, DEFAULT_KW).map { it.toString() }
val SIGMA_TELE_START_KWS = listOf(FIELD_KW, PROPERTY_KW).map { it.toString() }
...
val STRICT_KW_LIST = listOf(STRICT_KW.toString())
val HLEVELS_KW_LIST = listOf(HLEVELS_KW.toString())

val LOCAL_STATEMENT_KWS = STATEMENT_WT_KWS + TRUNCATED_DATA_KW_LIST
val GLOBAL_STATEMENT_KWS = STATEMENT_WT_KWS + TRUNCATED_DATA_KW_LIST + IMPORT_KW_LIST
