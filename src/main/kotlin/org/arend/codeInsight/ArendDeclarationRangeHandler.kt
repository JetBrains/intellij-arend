...
class ArendDeclarationRangeHandler : DeclarationRangeHandler<PsiElement> {
    override fun getDeclarationRange(container: PsiElement): TextRange =
            when (container) {
                is ArendDefFunction -> container.expr ?: container.nameTeleList.lastOrNull() as PsiElement?
                ?: container.defIdentifier
                is ArendDefData -> container.universeExpr ?: container.typeTeleList.lastOrNull() as PsiElement?
                ?: container.defIdentifier
                is ArendDefClass -> container.longNameList.lastOrNull()
