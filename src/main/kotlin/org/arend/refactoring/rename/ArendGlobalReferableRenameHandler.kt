...
class ArendGlobalReferableRenameHandler : MemberInplaceRenameHandler() {
    ...
    override fun isAvailable(element: PsiElement?, editor: Editor, file: PsiFile): Boolean {
        ...
        return e is GlobalReferable || e is ArendAliasIdentifier || (e is ArendDefIdentifier && e.parent is ArendNsId)
    }
    ...
    override fun invoke(project: Project, editor: Editor, file: PsiFile, dataContext: DataContext?) { //handles both dialog/inplace/test renames
        ...
        val isIDinAlias = elementAtCaret is LeafPsiElement && elementAtCaret.elementType == ID && elementAtCaret.psi.parent is ArendAliasIdentifier //TODO: \as-expression in NsId is also a kind of 'alias'
        if (isIDinAlias) {
    ...
    companion object {
        private fun customPerformDialogRename(elementToRename: PsiElement, editor: Editor) {
            val project = editor.project
            if (project != null && elementToRename is ReferableBase<*>) {
                val context = getContext(project, elementToRename, editor)
                if (context != null) {
                    val dialog = object: RenameDialog(project, elementToRename, if (context.isAlias) elementToRename.alias else elementToRename, editor) {
                        override fun createRenameProcessor(newName: String): RenameProcessor =
                                ArendRenameProcessor(project, elementToRename, newName, context.name, context.isAlias, null)
                    }
                    dialog.show()
                }
            }
        ...
        fun getContext(project: Project, elementToRename: PsiNameIdentifierOwner, editor: Editor): ArendRenameRefactoringContext? {
            if (elementToRename is GlobalReferable) {
                ...
                if (caretElement != null && elementToRename.hasAlias()) {
...
class ArendInplaceRenamer(elementToRename: PsiNamedElement,
    ...
    override fun collectRefs(referencesSearchScope: SearchScope?): MutableCollection<PsiReference> {
        ...
        if (aliasUnderCaret && element is ReferableBase<*>) {
