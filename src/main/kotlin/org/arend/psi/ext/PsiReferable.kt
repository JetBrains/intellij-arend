...
abstract class PsiReferableImpl(node: ASTNode) : ArendCompositeElementImpl(node), PsiReferable {
    ...
    override fun getNameIdentifier(): PsiElement? = findChildOfType<ArendDefIdentifier>()
...
where StubT : ArendNamedStub, StubT : StubElement<*> {
    ...
    override fun getNameIdentifier(): ArendCompositeElement? = getChildOfType<ArendDefIdentifier>()

    override fun getName(): String? = stub?.name ?: if (isValid) getChildOfType<ArendDefIdentifier>()?.referenceName else null
