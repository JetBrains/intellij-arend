...
import org.arend.module.starter.ArendStarterUtils.isDependencyUpdateFileExpired
...
open class ArendStarterInitialStep(contextProvider: ArendStarterContextProvider) : ArendCommonStarterInitialStep(
    ...
    protected val starterContext: ArendStarterContext = contextProvider.starterContext
    ...
    override fun validate(): Boolean {
        return validateFormFields(component, contentPanel, validatedTextComponents)
    }
    ...
    private fun checkDependencyUpdates(starters: List<Starter>) {
        for (starter in starters) {
            ...
            val dependencyConfig = parseDependencyConfig(dependencyUpdates, resourcePath)
    ...
    private fun loadStarterDependencyUpdatesFromFile(starterId: String): DependencyConfig? {
        ...
        if (!configUpdateFile.exists()
            || isDependencyUpdateFileExpired(configUpdateFile)
        ) {
            return null
        }
        ...
        return try {
            parseDependencyConfig(JDOMUtil.load(configUpdateFile), resourcePath)
        }
        catch (e: Exception) {
            logger<ArendStarterInitialStep>().warn("Failed to load starter dependency updates from file: $resourcePath. The file will be deleted.")
    ...
    private fun loadStarterDependencyUpdatesFromNetwork(starterId: String): Pair<Element, String>? {
        ...
        return try {
            ...
            if (e is HttpRequests.HttpStatusException
                ...
                logger<ArendStarterInitialStep>().debug("No updates for $starterId: $url")
            }
            else if (e is SocketTimeoutException) {
                logger<ArendStarterInitialStep>().debug("Socket timeout for $starterId: $url")
            }
            else {
                logger<ArendStarterInitialStep>().warn("Unable to load external starter $starterId dependency updates from: $url", e)
            }
            null
        }
    }
