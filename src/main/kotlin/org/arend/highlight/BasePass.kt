...
abstract class BasePass(protected open val file: IArendFile, editor: Editor, name: String, protected val textRange: TextRange, highlightInfoProcessor: HighlightInfoProcessor)
    ...
    fun reportToEditor(error: GeneralError, cause: PsiElement) {
        ...
        if (error is CannotFindConstructorError && cause is ArendPattern) {
            ...
            val references = when {
                ...
                refElement != null -> listOfNotNull(refElement.takeIf { it.longName.size == 1 }?.findChildOfType<ArendReferenceElement>())
                else -> cause.sequence.mapNotNull { subpattern -> subpattern.singleReferable ?: subpattern.referenceElement?.takeIf { it.longName.size == 1 }?.findChildOfType<ArendReferenceElement>() }
            }
    ...
    companion object {
        ...
        fun isEmptyGoal(element: PsiElement): Boolean {
            val goal: ArendGoal? = element.findChildOfType()
