...
import com.jetbrains.arend.ide.codeInsight.completion.ArdCompletionContributor
import com.jetbrains.arend.ide.psi.*
import com.jetbrains.arend.ide.psi.ext.ArdNewExprImplMixin
import com.jetbrains.arend.ide.psi.ext.impl.InstanceAdapter
import com.jetbrains.arend.ide.quickfix.InstanceQuickFix.Companion.INCREASE_IN_INDENT
import com.jetbrains.arend.ide.quickfix.InstanceQuickFix.Companion.moveCaretToTheEnd
...
interface ExpressionWithCoClauses {
    ...
    fun getCoClauseList(): List<ArdCoClause>
    fun calculateWhiteSpace(): String
    fun insertFirstCoClause(name: String, factory: ArdPsiFactory, editor: Editor?)
    fun isError(): Boolean
...
class InstanceQuickFix {
    companion object {
        ...
        private fun isEmptyGoal(element: PsiElement): Boolean {
            val goal: ArdGoal? = element.childOfType()
            return goal != null && ArdCompletionContributor.GOAL_IN_COPATTERN.accepts(goal)
        }

        private fun doAnnotate(expression: ExpressionWithCoClauses, classReference: ArdDefClass, holder: AnnotationHolder, onlyCheckFields: Boolean): Boolean {
        ...
        private fun annotateClauses(coClauseList: List<ArdCoClause>, holder: AnnotationHolder, superClassesFields: HashMap<ClassReferable, MutableSet<FieldReferable>>, fields: MutableSet<FieldReferable>) {
            val classClauses = ArrayList<Pair<ArdDefClass, ArdCoClause>>()
            for (coClause in coClauseList) {
                ...
                if (underlyingRef is ArdDefClass) {
                ...
                if (clauseBlock || emptyGoal) {
                    ...
                    if (typeClassReference is ArdDefClass) doAnnotate(object : CoClauseAdapter(coClause) {
                        ...
                        override fun insertFirstCoClause(name: String, factory: ArdPsiFactory, editor: Editor?) {
        ...
        fun annotateClassInstance(instance: InstanceAdapter, holder: AnnotationHolder): Boolean {
            ...
            if (classReference is ArdDefClass && classReference.recordKw == null) {
                ...
                if (argumentAppExpr != null) {
                    return doAnnotate(object : ExpressionWithCoClauses {
                        ...
                        override fun getCoClauseList(): List<ArdCoClause> = instance.coClauses?.coClauseList
                                ?: emptyList()
                        ...
                        override fun calculateWhiteSpace(): String {
                            ...
                            return if (instance.parent is ArdStatement && instance.parent.prevSibling is PsiWhiteSpace)
        ...
        fun annotateNewExpr(newExpr: ArdNewExprImplMixin, holder: AnnotationHolder): Boolean {
            val classReference = newExpr.classReference
            if (classReference is ArdDefClass) {
                ...
                if (argumentAppExpr != null) {
                    return doAnnotate(object : ExpressionWithCoClauses {
                        ...
                        override fun getCoClauseList(): List<ArdCoClause> = newExpr.getCoClauseList()
...
abstract class CoClauseAdapter(private val coClause: ArdCoClause) : ExpressionWithCoClauses {
    override fun getCoClauseList(): List<ArdCoClause> = coClause.coClauseList
...
class ImplementFieldsQuickFix(val instance: ExpressionWithCoClauses, private val fieldsToImplement: Collection<Referable>, private val actionText: String) : IntentionAction, Iconable {
    ...
    private fun addField(field: Referable, whitespace: String, editor: Editor?, psiFactory: ArdPsiFactory) {
    ...
    override fun invoke(project: Project, editor: Editor?, file: PsiFile?) {
        ...
        val psiFactory = ArdPsiFactory(project)
        ...
        if (instance.getCoClauseList().isNotEmpty()) { // Add CRLF + indent after the last coclause
            ...
            if (lastCC.nextSibling != null &&
                    lastCC.nextSibling.node.elementType == ArdElementTypes.RBRACE) {
