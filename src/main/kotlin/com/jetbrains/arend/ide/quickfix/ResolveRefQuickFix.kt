...
class ImportFileAction(private val importFile: ArdFile, private val currentFile: ArdFile, private val usingList: List<String>?) : ResolveRefFixAction {
    override fun toString() = "Import file " + importFile.fullName

    override fun isValid() = currentFile.module?.ardlFile?.findArdFile(importFile.modulePath) == importFile

    override fun execute(editor: Editor?) {
        val fullName = importFile.fullName
        val factory = ArdPsiFactory(importFile.project)
...
class AddIdToUsingAction(private val statCmd: ArdStatCmd, private val idList: List<String>) : ResolveRefFixAction {
    ...
    private fun addId(id: String) {
        if (statCmd.importKw != null) {
            ...
            if (using != null) {
                ...
                val factory = ArdPsiFactory(project)
...
class RemoveFromHidingAction(private val statCmd: ArdStatCmd, val id: ArdRefIdentifier) : ResolveRefFixAction {
    override fun toString(): String {
    ...
    override fun execute(editor: Editor?) {
        ...
        val leftEnd = startSibling.prevSibling.node.elementType == ArdElementTypes.LPAREN

        while (endSibling.nextSibling is PsiWhiteSpace || endSibling.nextSibling.node.elementType == ArdElementTypes.COMMA) {
            endSibling = endSibling.nextSibling
            if (endSibling.node.elementType == ArdElementTypes.COMMA && !leftEnd)
                break
        }

        val rightEnd = endSibling.nextSibling.node.elementType == ArdElementTypes.RPAREN

        if (rightEnd && startSibling.prevSibling.node.elementType == ArdElementTypes.COMMA) {
        ...
        if (leftEnd && rightEnd) {
            ...
            if (startSibling.prevSibling is PsiWhiteSpace)
                startSibling = startSibling.prevSibling
            if (startSibling.prevSibling.node.elementType == ArdElementTypes.HIDING_KW)
                startSibling = startSibling.prevSibling
            if (startSibling.prevSibling is PsiWhiteSpace)
                startSibling = startSibling.prevSibling
        }
...
class RenameReferenceAction(private val element: ArdReferenceElement, private val id: List<String>) : ResolveRefFixAction {
    override fun toString(): String {
        return "Rename " + element.text + " to " + LongName(id).toString()
    }

    override fun execute(editor: Editor?) {
        if (element.parent is ArdLongName) {
            val lName = LongName(id).toString()
            val factory = ArdPsiFactory(element.project)
...
class ResolveRefFixData(val target: PsiLocatedReferable,
    ...
    override fun toString(): String {
        return LongName(targetFullName).toString() + (if (target.containingFile is ArdFile)
            " in " + LongName((target.containingFile as ArdFile).modulePath.toList()).toString() else
            "")
    }
...
class ResolveRefQuickFix {
    companion object {
        ...
        fun statCmdName(statCmd: ArdStatCmd): String {
        ...
        fun getDecision(target: PsiLocatedReferable, element: ArdReferenceElement): ResolveRefFixData? {
            val targetFile = target.containingFile
            val currentFile = element.containingFile

            val fullName = ArrayList<String>()
            val alternativeFullName: ArrayList<String>? = if (target is ArdClassFieldSyn || target is ArdClassField || target is ArdConstructor)
                ArrayList() else
                null
            var ignoreFlag = true
            ...
            while (psi.parent != null) {
                if (psi is PsiReferable && psi !is ArdFile) {
            ...
            if (currentFile is ArdFile && targetFile is ArdFile) {
                if (currentFile != targetFile) {
                    ...
                    var suitableImport: ArdStatCmd? = null
                    ...
                    if (fullNames.isEmpty()) { // target definition is inaccessible in current context
                        ...
                        if (suitableImport != null) { // target definition is hidden or not included into using list but targetFile already has been imported
                            ...
                            for (fName in fullNames) {
                                val hiddenRef: ArdRefIdentifier? = hiddenList.lastOrNull { it.referenceName == fName[0] }
                                if (hiddenRef != null)
            ...
            if (fullName.size > 1) {
                val namespaceCommands = ArrayList<List<ArdStatCmd>>()
                psi = element
                while (psi.parent != null) {
                    var statements: List<ArdStatCmd>? = null

                    if (psi is ArdWhere)
                        statements = psi.children.mapNotNull { (it as? ArdStatement)?.statCmd }
                    else if (psi is ArdFile)
                    ...
                    if (modifyingImportsNeeded && targetTop.isNotEmpty()) { // calculate the scope imitating current scope after the imports have been fixed
                        val complementScope = object : ListScope(targetTop) {
                            override fun resolveNamespace(name: String?): Scope? = targetTop
                                    .filterIsInstance<ArdDefinition>()
