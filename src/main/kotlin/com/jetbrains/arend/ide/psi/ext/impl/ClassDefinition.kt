...
import com.jetbrains.arend.ide.psi.stubs.ArdDefClassStub
...
abstract class ClassDefinitionAdapter : DefinitionAdapter<ArdDefClassStub>, ArdDefClass, Abstract.ClassDefinition {
    constructor(node: ASTNode) : super(node)

    constructor(stub: ArdDefClassStub, nodeType: IStubElementType<*, *>) : super(stub, nodeType)
    ...
    private inline val parameterFields: List<ArdFieldDefIdentifier> get() = fieldTeleList.flatMap { it.fieldDefIdentifierList }
    ...
    override fun getSuperClasses(): List<ArdLongName> = longNameList

    override fun getClassFields(): List<Abstract.ClassField> = classStatList.mapNotNull { it.classField }

    override fun getClassFieldImpls(): List<ArdClassImplement> = classStatList.mapNotNull { it.classImplement }
    ...
    override fun getCoercingFunctions(): List<LocatedReferable> =
            (dynamicSubgroups + subgroups).mapNotNull { if (it is ArdDefFunction && it.coerceKw != null) it else null }
    ...
    override fun getIcon(flags: Int): Icon = ArdIcons.CLASS_DEFINITION
}
