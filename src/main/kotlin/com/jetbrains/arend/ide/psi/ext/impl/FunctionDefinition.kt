...
import com.jetbrains.arend.ide.psi.stubs.ArdDefFunctionStub
...
abstract class FunctionDefinitionAdapter : DefinitionAdapter<ArdDefFunctionStub>, ArdDefFunction, Abstract.FunctionDefinition {
    constructor(node: ASTNode) : super(node)

    constructor(stub: ArdDefFunctionStub, nodeType: IStubElementType<*, *>) : super(stub, nodeType)

    override fun getParameters(): List<ArdNameTele> = nameTeleList

    override fun getResultType(): ArdExpr? = expr

    override fun getTerm(): ArdExpr? = functionBody?.expr

    override fun getEliminatedExpressions(): List<ArdRefIdentifier> = functionBody?.elim?.refIdentifierList
            ?: emptyList()

    override fun getClauses(): List<ArdClause> = functionBody?.functionClauses?.clauseList ?: emptyList()
    ...
    override fun getIcon(flags: Int): Icon = ArdIcons.FUNCTION_DEFINITION
