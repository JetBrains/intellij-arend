...
class ArdReferenceImporter : ReferenceImporter {
    override fun autoImportReferenceAt(editor: Editor, file: PsiFile, offset: Int): Boolean {
        ...
        if (!file.viewProvider.languages.contains(com.jetbrains.arend.ide.ArdLanguage.INSTANCE))
            return false

        val element = file.findReferenceAt(offset)
        if (element is ArdReferenceElement) {
