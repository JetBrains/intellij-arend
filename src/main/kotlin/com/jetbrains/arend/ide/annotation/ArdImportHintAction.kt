...
import com.jetbrains.arend.ide.quickfix.ResolveRefFixData
import com.jetbrains.arend.ide.quickfix.ResolveRefQuickFix
...
class ArendImportHintAction(private val referenceElement: ArdReferenceElement) : HintAction, HighPriorityAction {
    ...
    private fun getItemsToImport(): List<ResolveRefFixData> {
        if (importQuickFixAllowed(referenceElement)) {
            ...
            if (preludeLibrary is ArdPreludeLibrary) {
            ...
            val items = StubIndex.getElements(ArdDefinitionIndex.KEY, name, project, ProjectAndLibrariesScope(project), PsiReferable::class.java).filterIsInstance<PsiLocatedReferable>().union(preludeItems.filterIsInstance(PsiLocatedReferable::class.java))
    ...
    override fun invoke(project: Project, editor: Editor, file: PsiFile?) {
        ...
        ApplicationManager.getApplication().runWriteAction {
            ...
            val action = ArdAddImportAction(project, editor, referenceElement, fixData)
            action.execute()
        }
    ...
    companion object {
        fun importQuickFixAllowed(referenceElement: ArdReferenceElement) =
                referenceElement is ArdSourceNode && referenceUnresolved(referenceElement) && ScopeFactory.isGlobalScopeVisible(referenceElement.topmostEquivalentSourceNode)

        fun referenceUnresolved(referenceElement: ArdReferenceElement): Boolean {
        ...
        fun getResolved(referenceElement: ArdReferenceElement): PsiElement? =
