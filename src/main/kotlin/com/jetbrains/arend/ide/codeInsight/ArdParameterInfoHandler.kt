...
import com.jetbrains.arend.ide.typing.parseBinOp
...
class ArdParameterInfoHandler : ParameterInfoHandler<Abstract.Reference, List<Abstract.Parameter>> {
    // private var lastAppExpr: ArdArgumentAppExpr? = null
...
//                vars.mapTo(nameTypeList) {
    ...
    override fun findElementForParameterInfo(context: CreateParameterInfoContext): Abstract.Reference? {
        ...
        val referable = ref?.referent?.let { resolveIfNeeded(it, (ref as ArdSourceNode).scope) }
    ...
    private fun fixedFindElement(file: PsiFile, offset: Int): PsiElement? {
        ...
        if (elem is PsiWhiteSpace) {
            if (elem.textOffset != offset) { //context.editor.caretModel.offset) {
                // return PsiTreeUtil.getParentOfType(PsiTreeUtil.nextLeaf(arg), ArdArgument::class.java, true, ArdArgumentAppExpr::class.java)
                return PsiTreeUtil.nextLeaf(elem)
            } else {
                // return PsiTreeUtil.getParentOfType(PsiTreeUtil.prevLeaf(arg), ArdArgument::class.java, true, ArdArgumentAppExpr::class.java)
    ...
    private fun findArgInParsedBinopSeq(arg: ArdExpr, expr: Concrete.Expression, curArgInd: Int, curFunc: Abstract.Reference?): Pair<Int, Abstract.Reference>? {
        if (expr is Concrete.ReferenceExpression || expr is Concrete.HoleExpression) {
            if ((expr.data as? ArdSourceNode)?.topmostEquivalentSourceNode == arg.topmostEquivalentSourceNode ||
                    (expr.data as? ArdSourceNode)?.parentSourceNode?.topmostEquivalentSourceNode == arg.topmostEquivalentSourceNode) {
                if (curFunc == null) {
    ...
    private fun locateArg(arg: ArdExpr, appExpr: ArdExpr) =
    ...
    private fun findAppExpr(file: PsiFile, offset: Int): Pair<Int, Abstract.Reference>? {
        ...
        /*
        val mbJumpToExternalAppExpr = lbl_@{arg:ArdArgument?, appExpr:ArdArgumentAppExpr ->
            if (extractParametersHolder(appExpr) == null) {
                if (arg != null || appExpr.parentSourceNode !is ArdArgument || appExpr.parentSourceNode?.parentSourceNode !is ArdArgumentAppExpr) {
                    return@lbl_ null
                }
                return@lbl_ Pair(appExpr.parentSourceNode as ArdArgument, appExpr.parentSourceNode!!.parentSourceNode as ArdArgumentAppExpr)
            }
            return@lbl_ Pair(arg, appExpr)
        }

        val processReference = lbl@{
            if (absNodeParent is ArdArgument) {
                val argLoc = locateArg(absNodeParent as ArdArgument)
                if (argLoc == null && absNodeParent.parentSourceNode?.parentSourceNode is ArdArgument) {
                    return@lbl locateArg(absNodeParent.parentSourceNode?.parentSourceNode as ArdArgument)
                }
                return@lbl argLoc
                /*if (absNodeParent.parentSourceNode !is ArdArgumentAppExpr) {
                    return@lbl null
                } else {
                    //return@lbl Pair(absNodeParent as? ArdArgument, absNodeParent!!.parentSourceNode as ArdArgumentAppExpr)
                    return@lbl mbJumpToExternalAppExpr(absNodeParent as? ArdArgument, absNodeParent.parentSourceNode as ArdArgumentAppExpr)
                } */
            } else if (absNodeParent is ArdArgumentAppExpr) {
                //return@lbl Pair(null, absNodeParent as ArdArgumentAppExpr)
                if (absNodeParent.parentSourceNode is ArdArgument) {
                    return@lbl locateArg(absNodeParent.parentSourceNode as ArdArgument)
                }

                return@lbl extractParametersHolder(absNodeParent as ArdArgumentAppExpr)?.let{ Pair(null, it)}
                //mbJumpToExternalAppExpr(null, absNodeParent as ArdArgumentAppExpr)
            }
            return@lbl null
        }*/
        ...
        if (absNode is ArdTypeTele || absNodeParent is ArdTypeTele) {
            return null
        }
        ...
        if (absNodeParent is ArdArgument && absNodeParent.parentSourceNode is ArdExpr) {
            var arg: ArdArgument = absNodeParent
            val argLoc = arg.expression?.let { locateArg(it as ArdExpr, absNodeParent.parentSourceNode as ArdExpr) }
            if (argLoc == null && absNodeParent.parentSourceNode?.parentSourceNode is ArdArgument) {
                arg = absNodeParent.parentSourceNode?.parentSourceNode as ArdArgument
                return arg.expression?.let { locateArg(it as ArdExpr, absNodeParent.parentSourceNode as ArdExpr) }
            }
            return argLoc
        } else if (absNodeParent is ArdArgumentAppExpr) {
            val argLoc = locateArg(absNode as ArdExpr, absNodeParent)
            if (argLoc != null) return argLoc

            if (absNodeParent.parentSourceNode is ArdArgument && absNodeParent.parentSourceNode?.parentSourceNode is ArdExpr) {
                val arg: ArdArgument = absNodeParent.parentSourceNode as ArdArgument
                return arg.expression?.let { locateArg(it as ArdExpr, absNodeParent.parentSourceNode?.parentSourceNode as ArdExpr) }
            }
    ...
    private fun extractParametersHolder(appExpr: ArdArgumentAppExpr): Abstract.ParametersHolder? {
