...
import com.jetbrains.arend.ide.psi.ext.impl.DefinitionAdapter
import com.jetbrains.arend.ide.search.ArdWordScanner
import com.jetbrains.jetpad.vclang.term.abs.Abstract
import java.util.*

class ArdCompletionContributor : CompletionContributor() {

    init {
        extend(CompletionType.BASIC, PREC_CONTEXT, KeywordCompletionProvider(FIXITY_KWS))

        extend(CompletionType.BASIC, afterLeaf(FAT_ARROW), originalPositionCondition(withParentOrGrandParent(ArdClassFieldSyn::class.java),
                KeywordCompletionProvider(FIXITY_KWS))) // fixity kws for class field synonym (2nd part)
        extend(CompletionType.BASIC, AS_CONTEXT, ProviderWithCondition({ parameters, _ -> (parameters.position.parent.parent as ArdNsId).asKw == null },
                KeywordCompletionProvider(AS_KW_LIST)))

        extend(CompletionType.BASIC, NS_CMD_CONTEXT, originalPositionCondition(withParent(ArdFile::class.java),
                KeywordCompletionProvider(HU_KW_LIST)))
        extend(CompletionType.BASIC, NS_CMD_CONTEXT, ProviderWithCondition({ parameters, _ -> noUsing(parameters.position.parent.parent as ArdStatCmd) },
                KeywordCompletionProvider(USING_KW_LIST)))
        extend(CompletionType.BASIC, NS_CMD_CONTEXT, ProviderWithCondition({ parameters, _ -> noUsingAndHiding(parameters.position.parent.parent as ArdStatCmd) },
                KeywordCompletionProvider(HIDING_KW_LIST)))
        extend(CompletionType.BASIC, withAncestors(PsiErrorElement::class.java, ArdNsUsing::class.java, ArdStatCmd::class.java),
                ProviderWithCondition({ parameters, _ -> noHiding(parameters.position.parent.parent.parent as ArdStatCmd) },
                        KeywordCompletionProvider(HIDING_KW_LIST)))

        val statementCondition = { psi: PsiElement -> psi is ArdStatement || psi is ArdClassStat }
        ...
        extend(CompletionType.BASIC, STATEMENT_END_CONTEXT, onJointOfStatementsCondition(statementCondition,
                ProviderWithCondition({ parameters, _ ->
                    ...
                    val ppp = pp.parent
                    pp.ancestors.filter { it is ArdWhere || it is ArdDefClass }.none() ||
                            (ppp is PsiErrorElement && ppp.parent is ArdDefClass && ppp.nextSibling == null)
                },
                        KeywordCompletionProvider(IMPORT_KW_LIST))))

        val coerceAdmissibleAncestors = or(and(ofTypeK(ArdWhere::class.java), withParents(ArdDefData::class.java, ArdDefClass::class.java)), ofTypeK(ArdDefClass::class.java))
        ...
        { jD: JointData ->
            ...
            while (anc != null && anc !is ArdDefinition && anc !is ArdDefModule && anc !is ArdClassStat) anc = anc.parent
            if (anc != null) {
                val da: ArdDefinition? = anc as? ArdDefinition /* TODO: create WhereHolder concept */
                val dm: ArdDefModule? = anc as? ArdDefModule
                (when {
        ...
        val noExtendsCondition = ProviderWithCondition({ cP, _ ->
            val dC = cP.position.ancestors.filterIsInstance<ArdDefClass>().firstOrNull()
            if (dC != null) dC.extendsKw == null else false
        }, KeywordCompletionProvider(EXTENDS_KW_LIST))

        extend(CompletionType.BASIC, and(withAncestors(PsiErrorElement::class.java, ArdDefClass::class.java), afterLeaf(ID)), noExtendsCondition)
        extend(CompletionType.BASIC, withAncestors(PsiErrorElement::class.java, ArdFieldTele::class.java, ArdDefClass::class.java),
                ProviderWithCondition({ parameters, _ ->
        ...
        val bareSigmaOrPi = { expression: PsiElement ->
            ...
            var tele: ArdTypeTele? = null
            while (result != null) {
                if (result is ArdTypeTele) tele = result
                if (result is ArdExpr && result !is ArdUniverseAtom) break
            ...
            if (context.accepts(expression)) true else
                if (tele?.text == null || tele.text.startsWith("(")) false else //Not Bare \Sigma or \Pi -- should display all expression keywords in completion
                    result is ArdSigmaExpr || result is ArdPiExpr
        }
        val noExpressionKwsAfter = ofType(SET, PROP_KW, UNIVERSE, TRUNCATED_UNIVERSE, NEW_KW)
        val afterElimVar = and(ofType(ID), withAncestors(ArdRefIdentifier::class.java, ArdElim::class.java))
        ...
        val expressionFilter = { basicCompletionProvider: CompletionProvider<CompletionParameters>, allowInBareSigmaOrPiExpressions: Boolean, allowInArgumentExpressionContext: Boolean ->
            genericJointCondition({ cP, _, jD ->
                ...
                !FIELD_CONTEXT.accepts(jD.prevElement) && //No keyword completion after field
                        ...
                        !(ofType(RBRACE).accepts(jD.prevElement) && withParent(ArdCaseExpr::class.java).accepts(jD.prevElement)) && //No keyword completion after \with or } in case expr
        ...
        extend(CompletionType.BASIC, LPH_CONTEXT, ProviderWithCondition({ cP, _ ->
            ...
            when (pp) {
                is ArdSetUniverseAppExpr, is ArdTruncatedUniverseAppExpr ->
                    pp.children.filterIsInstance<ArdAtomLevelExpr>().isEmpty()
                else -> pp.children.filterIsInstance<ArdAtomLevelExpr>().size <= 1
            }
        }, KeywordCompletionProvider(LPH_KW_LIST)))

        extend(CompletionType.BASIC, withParent(ArdLevelExpr::class.java), ProviderWithCondition({ cP, _ ->
        ...
        val pairingInCondition = { pos: PsiElement -> pairingWordCondition({ position: PsiElement? -> position is ArdLetExpr && position.inKw == null }, pos) }
        val pairingWithCondition = { pos: PsiElement -> pairingWordCondition({ position: PsiElement? -> position is ArdCaseExpr && position.withKw == null }, pos) }
        ...
        val asCondition1 = { position: PsiElement? -> position is ArdCaseArg && position.asKw == null }
        ...
        val returnCondition = { pos: PsiElement ->
            pairingWordCondition({ position: PsiElement? ->
                if (position is ArdCaseArg) {
                    val pp = position.parent as? ArdCaseExpr
        ...
        extend(CompletionType.BASIC, ELIM_CONTEXT, ProviderWithCondition({ cP, _ ->
            ...
            while (pos2 != null) {
                if (pos2.nextSibling is PsiWhiteSpace) {
                    if ((pos2.nextSibling.nextSibling is ArdFunctionBody) && (pos2.parent is ArdDefFunction)) {
                        val fBody = (pos2.parent as ArdDefFunction).functionBody
                        exprFound = fBody == null || fBody.fatArrow == null && fBody.elim?.elimKw == null
                        exprFound = exprFound && !emptyTeleList((pos2.parent as ArdDefFunction).nameTeleList) // No point of writing elim keyword if there are no arguments
                        break
                    }
                    if ((pos2.nextSibling.nextSibling is ArdDataBody) && (pos2.parent is ArdDefData)) {
                        val dBody = (pos2.parent as ArdDefData).dataBody
                        ...
                        exprFound = exprFound && !emptyTeleList((pos2.parent as ArdDefData).typeTeleList)
                        break

                    }
                }
                if (pos2 is ArdConstructor && pos2.elim == null) {
        ...
        val isLiteralApp = { argumentAppExpr: ArdArgumentAppExpr ->
            argumentAppExpr.longNameExpr != null ||
                    ((argumentAppExpr.children[0] as? ArdAtomFieldsAcc)?.atom?.literal?.longName != null)
        }
        ...
        val unifiedLevelCondition = { atomIndex: Int?, forbidLevelExprs: Boolean, threshold: Int ->
            { cP: CompletionParameters, _: ProcessingContext? ->
                var anchor: PsiElement? = if (atomIndex != null) cP.position.ancestors.filterIsInstance<ArdAtomFieldsAcc>().elementAtOrNull(atomIndex) else null
                var argumentAppExpr: ArdArgumentAppExpr? =
                        (anchor?.parent as? ArdAtomArgument)?.parent as? ArdArgumentAppExpr
                                ?: anchor?.parent as? ArdArgumentAppExpr
                if (anchor == null) {
                    anchor = cP.position.parent
                    argumentAppExpr = anchor?.parent as? ArdArgumentAppExpr
                ...
                if (anchor == null) {
                    argumentAppExpr = cP.position.parent as? ArdArgumentAppExpr
                }
                ...
                if (argumentAppExpr != null && anchor != null && isLiteralApp(argumentAppExpr)) {
                    ...
                    for (ch in argumentAppExpr.children) {
                        ...
                        if (ch is ArdAtomArgument) forbidden = true
                    }
        ...
        extend(CompletionType.BASIC, withAncestors(PsiErrorElement::class.java, ArdArgumentAppExpr::class.java),
                ProviderWithCondition(unifiedLevelCondition.invoke(null, false, 2), KeywordCompletionProvider(LPH_LEVEL_KWS)))

        extend(CompletionType.BASIC, withParent(ArdArgumentAppExpr::class.java), ProviderWithCondition({ cP, _ ->
            val argumentAppExpr: ArdArgumentAppExpr? = cP.position.parent as ArdArgumentAppExpr
    ...
    companion object {
        ...
        val PREC_CONTEXT = or(afterLeaf(FUNCTION_KW), afterLeaf(COERCE_KW), afterLeaf(DATA_KW), afterLeaf(CLASS_KW), afterLeaf(RECORD_KW), and(afterLeaf(AS_KW), withGrandParent(ArdNsId::class.java)),
                and(afterLeaf(PIPE), withGrandParents(ArdConstructor::class.java, ArdDataBody::class.java)), //simple data type constructor
                and(afterLeaf(FAT_ARROW), withGrandParents(ArdConstructor::class.java, ArdConstructorClause::class.java)), //data type constructors with patterns
                and(afterLeaf(PIPE), withGrandParents(ArdClassField::class.java, ArdClassStat::class.java)), //class field
                and(afterLeaf(FAT_ARROW), withGrandParent(ArdClassFieldSyn::class.java))) //class field synonym

        val AS_CONTEXT = and(withGrandParent(ArdNsId::class.java), withParents(ArdRefIdentifier::class.java, PsiErrorElement::class.java))
        val NS_CMD_CONTEXT = withAncestors(PsiErrorElement::class.java, ArdStatCmd::class.java)
        val ANY: PsiElementPattern.Capture<PsiElement> = PlatformPatterns.psiElement()
        val STATEMENT_END_CONTEXT = withParents(PsiErrorElement::class.java, ArdRefIdentifier::class.java)
        val WHERE_CONTEXT = and(or(STATEMENT_END_CONTEXT, withAncestors(ArdDefIdentifier::class.java, ArdIdentifierOrUnknown::class.java, ArdNameTele::class.java)),
                ...
                not(withAncestors(PsiErrorElement::class.java, ArdDefInstance::class.java)), // don't allow \where in incomplete instance expressions
                not(withAncestors(ArdDefIdentifier::class.java, ArdIdentifierOrUnknown::class.java, ArdNameTele::class.java, ArdDefInstance::class.java))
        )
        val DATA_CONTEXT = withAncestors(PsiErrorElement::class.java, ArdDefData::class.java, ArdStatement::class.java)
        val EXPRESSION_CONTEXT = or(withAncestors(ArdRefIdentifier::class.java, ArdLongName::class.java, ArdLiteral::class.java, ArdAtom::class.java),
                withParentOrGrandParent(ArdFunctionBody::class.java),
                withParentOrGrandParent(ArdExpr::class.java),
                withAncestors(PsiErrorElement::class.java, ArdClause::class.java),
                and(ofType(INVALID_KW), afterLeaf(COLON), withParent(ArdNameTele::class.java)))
        val CASE_CONTEXT = or(EXPRESSION_CONTEXT, withAncestors(PsiErrorElement::class.java, ArdCaseArg::class.java, ArdCaseExpr::class.java))
        val FIELD_CONTEXT = withAncestors(ArdFieldAcc::class.java, ArdAtomFieldsAcc::class.java)
        val TELE_CONTEXT =
                or(and(withAncestors(PsiErrorElement::class.java, ArdTypeTele::class.java),
                        withGreatGrandParents(ArdClassField::class.java, ArdConstructor::class.java, ArdDefData::class.java, ArdPiExpr::class.java, ArdSigmaExpr::class.java)),
                        withAncestors(ArdRefIdentifier::class.java, ArdLongName::class.java, ArdLiteral::class.java, ArdTypeTele::class.java))
        val FIRST_TYPE_TELE_CONTEXT = and(afterLeaf(ID), withParent(PsiErrorElement::class.java),
                withGrandParents(ArdDefData::class.java, ArdClassField::class.java, ArdConstructor::class.java))

        val DATA_OR_EXPRESSION_CONTEXT = or(DATA_CONTEXT, EXPRESSION_CONTEXT, TELE_CONTEXT, FIRST_TYPE_TELE_CONTEXT)
        val ARGUMENT_EXPRESSION = or(withAncestors(ArdRefIdentifier::class.java, ArdLongName::class.java, ArdLiteral::class.java, ArdAtom::class.java, ArdAtomFieldsAcc::class.java, ArdAtomArgument::class.java),
                withAncestors(PsiErrorElement::class.java, ArdAtomFieldsAcc::class.java, ArdArgumentAppExpr::class.java))
        val LPH_CONTEXT = and(withParent(PsiErrorElement::class.java), withGrandParents(ArdSetUniverseAppExpr::class.java, ArdUniverseAppExpr::class.java, ArdTruncatedUniverseAppExpr::class.java))
        val LPH_LEVEL_CONTEXT = and(withAncestors(PsiErrorElement::class.java, ArdAtomLevelExpr::class.java))
        val ELIM_CONTEXT = and(not(or(afterLeaf(DATA_KW), afterLeaf(FUNCTION_KW), afterLeaf(COERCE_KW), afterLeaf(TRUNCATED_KW), afterLeaf(COLON))),
                or(EXPRESSION_CONTEXT, TELE_CONTEXT,
                        withAncestors(ArdDefIdentifier::class.java, ArdIdentifierOrUnknown::class.java, ArdNameTele::class.java),
                        and(withParent(PsiErrorElement::class.java), withGrandParents(ArdNameTele::class.java, ArdTypeTele::class.java, ArdDefData::class.java, ArdDefFunction::class.java))))
        val ARGUMENT_EXPRESSION_IN_BRACKETS =
                withAncestors(ArdRefIdentifier::class.java, ArdLongName::class.java, ArdLiteral::class.java, ArdAtom::class.java,
                        ArdAtomFieldsAcc::class.java, ArdArgumentAppExpr::class.java, ArdNewExpr::class.java, ArdTupleExpr::class.java, ArdTuple::class.java,
                        ArdAtom::class.java, ArdAtomFieldsAcc::class.java, ArdAtomArgument::class.java, ArdArgumentAppExpr::class.java)
        val INSTANCE_CONTEXT = withAncestors(ArdRefIdentifier::class.java, ArdLongName::class.java, ArdLiteral::class.java, ArdAtom::class.java, ArdAtomFieldsAcc::class.java, ArdArgumentAppExpr::class.java, ArdDefInstance::class.java)
        val GOAL_IN_COPATTERN = ArdCompletionContributor.withAncestors(ArdLiteral::class.java, ArdAtom::class.java, ArdAtomFieldsAcc::class.java,
                ArdArgumentAppExpr::class.java, ArdNewExpr::class.java, ArdCoClause::class.java)

        private fun noUsing(cmd: ArdStatCmd): Boolean = cmd.nsUsing?.usingKw == null
        private fun noHiding(cmd: ArdStatCmd): Boolean = cmd.hidingKw == null
        private fun noUsingAndHiding(cmd: ArdStatCmd): Boolean = noUsing(cmd) && noHiding(cmd)
        ...
        private fun parentIsStatementHolder(p: PsiElement?) = when (p?.parent) {
            is ArdWhere -> true
            is ArdDefClass -> (p.parent as ArdDefClass).fatArrow == null
            else -> false
        }
        ...
        private fun onJointOfStatementsCondition(statementCondition: (PsiElement) -> Boolean, completionProvider: CompletionProvider<CompletionParameters>,
                ...
                genericJointCondition({ _, _, jointData ->
                    ...
                    val isInsideClassFields = (leftStatement is ArdClassStat && leftStatement.definition == null) &&
                            rightStatement is ArdClassStat && rightStatement.definition == null
    ...
    open class KeywordCompletionProvider(private val keywords: List<String>) : CompletionProvider<CompletionParameters>() {
        ...
        open fun computePrefix(parameters: CompletionParameters, resultSet: CompletionResultSet): String {
            ...
            val lastInvalidIndex = prefix.mapIndexedNotNull { i, c -> if (!ArdWordScanner.isArendIdentifierPart(c)) i else null }.lastOrNull()
