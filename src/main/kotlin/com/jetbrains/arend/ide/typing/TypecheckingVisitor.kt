...
import com.jetbrains.arend.ide.typing.ExpectedTypeVisitor.Companion.hasCoerce
...
class TypecheckingVisitor(private val element: ArdCompositeElement, private val holder: AnnotationHolder) : AbstractExpressionVisitor<Any, Unit> {
    companion object {
        fun resolveReference(data: Any?, referent: Referable) =
                if (data is ArdCompositeElement) {
                    if (data !is ArdLongName || data.refIdentifierList.size <= 1) {
                        Concrete.ReferenceExpression(data, data.reference?.resolve() as? Referable
    ...
    private fun compare(actualType: Abstract.Expression, expectedType: Any) {
        ...
        when (expectedType) {
            ...
            is ExpectedTypeVisitor.Definition ->
                if (!(expectedType.def is ArdDefData && actualDef == expectedType.def || expectedType.def is ArdDefClass && actualDef is ArdDefClass && actualDef.isSubClassOf(expectedType.def))) {
