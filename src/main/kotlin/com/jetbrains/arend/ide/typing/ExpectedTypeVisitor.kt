package com.jetbrains.arend.ide.typing
...
import com.jetbrains.arend.ide.psi.ext.ArdCoClauseImplMixin
import com.jetbrains.arend.ide.psi.ext.impl.ClassFieldImplAdapter
...
class ExpectedTypeVisitor(private val element: ArdExpr, private val holder: AnnotationHolder?) : AbstractExpressionVisitor<Void, Any>, AbstractDefinitionVisitor<Any> {
    ...
    class Substituted(val expr: Abstract.Expression) {
        override fun toString(): String = (expr as? ArdExpr)?.text ?: expr.toString()
    }

    class Definition(val def: ArdDefinition) {
    ...
    companion object {
        fun hasCoerce(def: ArdDefinition?, fromOther: Boolean, coerceType: CoerceType): Boolean {
            if (def == null || def !is ArdDefClass && def !is ArdDefData) {
                return false
            }

            if (!fromOther && def is ArdDefClass) {
            ...
            for (stat in stats) {
                ...
                if (statDef is ArdDefFunction && statDef.coerceKw != null) {
                    if (coerceType == CoerceType.ANY) {
                        return true
                    }
                ...
                while (currentType != null && requiredParameters > 0) {
                    ...
                    if (kind != GetKindVisitor.Kind.PI) {
                        if (holder != null && kind.isWHNF()) {
                            holder.createErrorAnnotation(TextRange((parameters.first() as ArdNameTele).textRange.startOffset, (parameters.last() as ArdNameTele).textRange.endOffset), "Too many parameters")
                        }
                        currentType = null
                        break
                    }
    ...
    fun getExpectedType(): Any? {
        ...
        return when (parent) {
            ...
            is Abstract.FunctionClause -> {
                ...
                when (pparent) {
                    is Abstract.FunctionDefinition -> (pparent.resultType as? ArdExpr)?.let { Substituted(it) }
                    is ArdConstructor -> pparent.ancestors.filterIsInstance<ArdDefData>().firstOrNull()?.let { Definition(it) }
                    else -> null
                }
            }
            is Abstract.ClassFieldImpl -> {
                val implemented = when (parent) {
                    ...
                    is ArdCoClauseImplMixin -> parent.getResolvedImplementedField()
                    else -> null
                }
                when (implemented) {
                    is ArdDefClass -> {
                        val parameters = PsiTreeUtil.getChildrenOfTypeAsList(parent as? PsiElement, ArdNameTele::class.java)
                        if (!parameters.isEmpty()) {
                    ...
                    is Abstract.ClassField -> reduceParameters(PsiTreeUtil.getChildrenOfTypeAsList(parent as? PsiElement, ArdNameTele::class.java), implemented.resultType, holder)
                    else -> null
                }
            }
    ...
    override fun visitLam(data: Any?, parameters: Collection<Abstract.Parameter>, body: Abstract.Expression?, errorData: Abstract.ErrorData?, params: Void?): Any? {
        ...
        val expectedType = (data as? ArdExpr)?.let { ExpectedTypeVisitor(it, null).getExpectedType() } ?: return null
    ...
    override fun visitGoal(data: Any?, name: String?, expression: Abstract.Expression?, errorData: Abstract.ErrorData?, params: Void?) =
            (data as? ArdExpr)?.let { ExpectedTypeVisitor(it, null).getExpectedType() }
    ...
    override fun visitTuple(data: Any?, fields: Collection<Abstract.Expression>, errorData: Abstract.ErrorData?, params: Void?): Any? {
        ...
        val sigmaExpr = (data as? ArdExpr)?.let { ExpectedTypeVisitor(it, null).getExpectedType() } as? ArdSigmaExpr
                ?: return null
    ...
    override fun visitLet(data: Any?, clauses: Collection<Abstract.LetClause>, expression: Abstract.Expression?, errorData: Abstract.ErrorData?, params: Void?) =
            if (element == expression) (data as? ArdExpr)?.let { ExpectedTypeVisitor(it, null).getExpectedType() } else null
