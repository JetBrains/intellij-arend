...
import com.jetbrains.arend.ide.resolving.ArdReferableConverter
import com.jetbrains.arend.ide.resolving.ArdResolveCache
import com.jetbrains.arend.ide.typechecking.error.LogErrorReporter
...
interface TypeCheckingService {
    ...
    fun getTypechecked(definition: ArdDefinition): Definition?
...
class TypeCheckingServiceImpl(override val project: Project) : TypeCheckingService {
    ...
    override val libraryManager = LibraryManager(ArdLibraryResolver(project), EmptyModuleScopeProvider.INSTANCE, null, libraryErrorReporter, libraryErrorReporter)

    private val simpleReferableConverter = SimpleReferableConverter()
    override val referableConverter: ReferableConverter
        get() = ArdReferableConverter(project, simpleReferableConverter)
            ...
            if (fileName.endsWith(FileUtils.LIBRARY_EXTENSION)) {
                ...
                if (fileName == module.name + FileUtils.LIBRARY_EXTENSION) {
                    ...
                    if (root != null && root == newParent) {
                        libraryManager.loadLibrary(ArdRawLibrary(module, typecheckerState))
                    }
                }
            }
        }
    ...
    private inner class TypeCheckerPsiTreeChangeListener : PsiTreeChangeAdapter() {
        ...
        override fun beforeChildRemoval(event: PsiTreeChangeEvent) {
            if (event.child is ArdFile) { // whole file has been removed
        ...
        private fun processParent(event: PsiTreeChangeEvent, checkCommentStart: Boolean) {
            ...
            if (checkCommentStart) {
                var node = (child as? ArdCompositeElement)?.node ?: child as? LeafPsiElement
            ...
            val definition = ancestors.filterIsInstance<ArdDefinition>().firstOrNull() ?: return
...
private fun isComment(element: IElementType) = element == ArdElementTypes.BLOCK_COMMENT || element == ArdElementTypes.LINE_COMMENT
