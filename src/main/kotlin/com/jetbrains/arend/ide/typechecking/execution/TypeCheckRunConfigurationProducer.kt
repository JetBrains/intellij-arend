...
import com.jetbrains.arend.ide.psi.parentOfType
import com.jetbrains.arend.ide.typechecking.execution.configurations.TypeCheckConfiguration
import com.jetbrains.arend.ide.typechecking.execution.configurations.TypeCheckConfigurationType
...
class TypeCheckRunConfigurationProducer : RunConfigurationProducer<TypeCheckConfiguration>(TypeCheckConfigurationType()) {
    ...
    private fun configurationFromContext(context: ConfigurationContext, sourceElement: Ref<PsiElement>?): MyConfiguration? {
        ...
        val definition = element?.parentOfType<ArdDefinition>(false) ?: element?.parentOfType<ArdFile>(false)
        when (definition) {
            is ArdDefinition -> {
                val file = definition.containingFile as? ArdFile ?: return null
