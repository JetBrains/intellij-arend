...
import com.jetbrains.arend.ide.psi.ArdStatement
import com.jetbrains.arend.ide.psi.ext.PsiLocatedReferable
import com.jetbrains.arend.ide.psi.findGroupByFullName
...
import com.jetbrains.arend.ide.typechecking.TestBasedTypechecking
import com.jetbrains.arend.ide.typechecking.TypeCheckingService
import com.jetbrains.arend.ide.typechecking.error.ParserError
import com.jetbrains.arend.ide.typechecking.error.TypecheckingErrorReporter
...
class TypeCheckProcessHandler(
    ...
    override fun startNotify() {
        ...
        val libraries = if (command.library == "" && modulePath == null) registeredLibraries.filterIsInstance<ArdRawLibrary>() else {
                ...
                for (library in libraries) {
                    ...
                    if (command.definitionFullName == "") {
                        ...
                        val ref = runReadAction {
                            ...
                            if (group is ArdDefClass && group.fatArrow != null) null else group?.referable
                        }
                        if (ref == null) {
    ...
    private fun reportParserErrors(group: PsiElement, module: ArdFile, typecheckingErrorReporter: TypecheckingErrorReporter) {
        for (child in group.children) {
            when (child) {
                ...
                is ArdStatement -> child.definition?.let { reportParserErrors(it, module, typecheckingErrorReporter) }
            }
        }
    }
