...
import com.jetbrains.arend.ide.psi.ext.PsiConcreteReferable
...
import com.jetbrains.arend.ide.typechecking.execution.TypecheckingEventsProcessor
...
class PsiConcreteProvider(private val project: Project, private val referableConverter: ReferableConverter, private val errorReporter: ErrorReporter, private val eventsProcessor: TypecheckingEventsProcessor?) : ConcreteProvider {
    ...
    private fun getConcreteDefinition(psiReferable: PsiConcreteReferable): Concrete.ReferableDefinition? {
        ...
        val result = cache.computeIfAbsent(psiReferable) {
            runReadAction {
                if (psiReferable is ArdDefClass && psiReferable.fatArrow != null) {
    ...
    override fun getConcreteFunction(referable: GlobalReferable): Concrete.FunctionDefinition? {
        ...
        return if (psiReferable is ArdDefFunction) getConcreteDefinition(psiReferable) as? Concrete.FunctionDefinition else null
    }

    override fun getConcreteInstance(referable: GlobalReferable): Concrete.Instance? {
        val psiReferable = PsiLocatedReferable.fromReferable(referable)
        return if (psiReferable is ArdDefInstance) getConcreteDefinition(psiReferable) as? Concrete.Instance else null
    }

    override fun getConcreteClass(referable: ClassReferable): Concrete.ClassDefinition? {
        val psiReferable = PsiLocatedReferable.fromReferable(referable)
        return if (psiReferable is ArdDefClass) getConcreteDefinition(psiReferable) as? Concrete.ClassDefinition else null
    }

    override fun getConcreteData(referable: GlobalReferable): Concrete.DataDefinition? {
        val psiReferable = PsiLocatedReferable.fromReferable(referable)
        return if (psiReferable is ArdDefData) getConcreteDefinition(psiReferable) as? Concrete.DataDefinition else null
    }

    override fun isRecord(classRef: ClassReferable): Boolean {
        val psiReferable = PsiLocatedReferable.fromReferable(classRef)
        return psiReferable is ArdDefClass && runReadAction { psiReferable.recordKw != null }
    }
    ...
    override fun isCoerce(ref: GlobalReferable): Boolean {
        ...
        return psiReferable is ArdDefFunction && runReadAction { psiReferable.coerceKw != null }
    }
}