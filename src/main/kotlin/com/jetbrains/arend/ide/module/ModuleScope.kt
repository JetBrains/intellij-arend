...
class ModuleScope private constructor(private val module: Module, private val rootDirs: List<VirtualFile>?) : Scope {
    ...
    override fun getElements(): Collection<Referable> {
        ...
        for (root in rootDirs ?: module.ardlFile?.sourcesDirFile?.let { listOf(it) } ?: emptyList()) {
            for (file in root.children) {
                if (file.isDirectory) {
                    ...
                    (psiManager.findFile(file) as? ArdFile)?.let { result.add(PsiModuleReferable(listOf(it), it.modulePath)) }
                }
            }
        }
        return result
    ...
    override fun resolveNamespace(name: String): Scope {
        val newRootDirs = (rootDirs ?: module.ardlFile?.sourcesDirFile?.let { listOf(it) }
        ?: emptyList()).mapNotNull { root ->
            for (file in root.children) {
                if (file.name == name) {
