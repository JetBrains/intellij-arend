...
class ArdRawLibrary(private val module: Module, typecheckerState: TypecheckerState) : SourceLibrary(typecheckerState) {
    private var headerFilePtr: SmartPsiElementPointer<ArdlFile>? = null

    val headerFile: ArdlFile?
        get() = headerFilePtr?.element

    override fun getName() = module.name

    override fun getModuleGroup(modulePath: ModulePath) = headerFilePtr?.element?.findArdFile(modulePath)
    ...
    override fun loadHeader(errorReporter: ErrorReporter): LibraryHeader {
        ...
        */
        //else if (headerVirtualFile.fileType == ArdlFileType) {
        headerFilePtr = module.ardlFile?.let { SmartPointerManager.getInstance(module.project).createSmartPsiElementPointer(it) }
            ...
            for (dep in deps) {
                WriteAction.run<Exception> {
                    ...
                    if (file != null) {
                        ...
                        libraryModel.commit()
                        tableModel.commit()
                        ModuleRootModificationUtil.addDependency(module.project.ardModules.elementAt(0), library)
                    }
                }
            }
        }*/
    ...
    override fun getRawSource(modulePath: ModulePath) = headerFilePtr?.element?.findArdFile(modulePath)?.let { ArdRawSource(it) }
            ?: ArdFakeRawSource(modulePath)
