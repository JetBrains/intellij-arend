...
class InteractiveGoalSolverQuickFixTest : QuickFixTestBase() {
    fun `test caret is moved to the first goal`() = doTestWithGoalSolver(TestGoalSolver("suc {?}", project), """
        \func test : Nat => {-caret-}{?}
    """, """
        \func test : Nat => suc {-caret-}{?}
    """)

    fun `test caret is not moved when no goals`() = doTestWithGoalSolver(TestGoalSolver("zero", project), """
        \func test : Nat => {-caret-}{?}
    """, """
    ...
    class TestGoalSolver(@Language("Arend") val solution: String, val project: Project) : InteractiveGoalSolver {
        ...
        override fun solve(typechecker: ExpressionTypechecker,
